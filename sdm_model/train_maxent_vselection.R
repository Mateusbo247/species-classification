...
for (model in models){
	...
	for(BATCH in 1:max_batch){
		...
		output_dir <- paste0('/home/antonio/mateus_results/maxent_vselection/train_results/')
		...
		for(FOLD in 1:max_fold){
			...
			test_csv <- paste0('/home/antonio/replicacao_mateus/dataset/train_registros_species.csv')
			...
			write.csv(result_all,  paste0(path, model, '_train_fold', FOLD,'.csv'), row.names = FALSE)
		...
		write.csv(df_geral, paste0(path, model, '_train_global.csv'), row.names = FALSE)
	...
	dir.create('/home/antonio/mateus_results/maxent_vselection/train/', showWarnings = FALSE)
	write.csv(acc_mat, paste0('/home/antonio/mateus_results/maxent_vselection/train/', model, '_acc_train_results.csv'), row.names = FALSE)
	write.csv(f1s_mat, paste0('/home/antonio/mateus_results/maxent_vselection/train/', model, '_f1s_train_results.csv'), row.names = FALSE)
}
